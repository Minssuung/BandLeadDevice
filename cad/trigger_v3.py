...
# ── 레버 (월드좌표, rest) ──
# ── 케이지: 허브 가운데 갭에 토션스프링 코일(4.8mm) 가둠 + 플랜지로 보스까지 공간 채움 (리프트와 동일) ──
COIL_LEN = 4.8; GAP_C = COIL_LEN + 0.4; FL_C = 3.9; HW_C = GAP_C / 2 + FL_C   # 갭5.2, 허브반폭6.5 (보스 x7까지 0.5)
hub = cq.Workplane("YZ", origin=(-HW_C, C[1], C[2])).circle(5).extrude(2 * HW_C)   # Ø10 허브 x±6.5 (축방향 공간 채움)
arm = cq.Workplane("XY", origin=(0, -25.5, -16)).box(10, 4, 22)
lever = hub.union(arm)
lever = lever.cut(cq.Workplane("XY", origin=(0, C[1] + 3.75, C[2])).box(GAP_C, 12.5, 6))   # 코일 케이지 갭 x±2.6, 뒤(+Y)로 개방, 앞 허브브리지 살림
lever = lever.cut(cq.Workplane("YZ", origin=(-7.5, C[1], C[2])).circle((PT.TRIG_PIVOT_DIA + 0.4) / 2).extrude(15))   # 피벗 보어 Φ3.4 (넓은허브 관통 x±7.5)
# 자석 포켓 Φ5.2×2.2 — rear(+Y)면에서 -Y로 파냄(자석이 +Y=홀쪽을 향함). z-18이라 케이지갭(z-6~12) 무관
...
lever = lever.cut(cq.Workplane("XY", origin=(0, C[1] - 0.5, C[2] - 5)).box(1.8, 4, 2.2))   # 토션스프링 레버다리 슬롯 (갭 가운데 중앙정렬, 암 윗면)
