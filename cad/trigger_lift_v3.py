#!/usr/bin/env python3
"""리프트 = 앞면 2단 트리거 레버 (중지). 당기면 뒤 nub이 SS-5GL 누름. SS-5GL 자체스프링이 복귀.
...
# ── 레버 ──
hub = cq.Workplane("YZ", origin=(-5, C[1], C[2])).circle(4).extrude(10)               # 허브 x-5..5
...
# ── 검증: 당김 전구간 그립과 간섭(걸림) + nub이 SS-5GL 누르나 ──
...
print(f"nub 뒤끝 y: rest {nub_rest:.1f} → pull {nub_pull:.1f} (SS-5GL 버튼 y≈1.6, pull때 닿아야)")
...
ax.add_patch(plt.Rectangle((SS[1] - 5.4, SS[2] - 10.3), 10.8, 20.6, fill=False, ec="green", lw=1.3))
ax.text(SS[1], SS[2] + 12, "SS-5GL", color="green", fontsize=8, ha="center")
...
ax.set_title("리프트 2단 트리거 (당기면 nub이 SS-5GL 누름)", fontsize=10); ax.legend(fontsize=8); ax.grid(alpha=.3)
...
fig.suptitle("리프트 2단 트리거 (중지) — SS-5GL 누름")
