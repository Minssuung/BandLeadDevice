    ...
    def run(self) -> None:
        ...
        try:
            while self._running and self._serial and self._serial.is_open:
                ...
                if chunk:
                    for _, _, latest in self._parser.feed(chunk):
...
class ImuWithMotorWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        # IMU 상태
        self.imu_reader:    Optional[SerialReaderThread] = None
    ...
    def _build_imu_panel(self) -> QWidget:
        ...
        row.addWidget(self.imu_port_combo)

        row.addWidget(QLabel("Baud"))
        self.imu_baud_combo = QComboBox()
        self.imu_baud_combo.addItems(["9600", "115200", "57600", "38400"])
        self.imu_baud_combo.setCurrentText("115200")
        row.addWidget(self.imu_baud_combo)
    ...
    def _refresh_ports(self) -> None:
        ...
        for p in ports:
            ...
            self.imu_port_combo.addItem(text, p.device)
            self.motor_port_combo.addItem(text, p.device)
        if not ports:
            self.imu_port_combo.addItem("사용 가능한 포트 없음", "")
    ...
    def _imu_connect(self) -> None:
        if self.imu_connected:
            return
        port = self.imu_port_combo.currentData()
        if not port:
        ...
        self.imu_reader = SerialReaderThread(port, baud)
    ...
    def _relay_to_motors(self, data: dict) -> None:
        ...
        mapping = {
            ...
            for motor_id, angle in raw_mapping.items()
        }

        parts   = []
        for motor_id, angle in mapping.items():
            delta_pos = int(angle / 360.0 * 4095)
            ...
            if w.torque_btn.isChecked():
                ...
                w.slider.blockSignals(False)
                w.spin.blockSignals(False)
            parts.append(f"ID{motor_id}: d{angle:+.1f}°→{pos}")
